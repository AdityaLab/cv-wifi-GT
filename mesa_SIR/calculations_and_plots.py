...
def compute(model, report):
    ...
    if report == "R0":
    ...
    elif report =='new asym':
        for a in model.schedule.agents:
            if (getattr(a, "asymptomatic") ==True):
                if model.asym[a.pos] ==0:
                    total +=1
        ...
        for a in model.schedule.agents:
            if (getattr(a, "recovered") ==True) or (getattr(a, "symptomatic") ==True) or (getattr(a, "isolation") ==True) or (getattr(a, "alive") ==False) :
                if model.SIRD[a.pos]==0:
                    total +=1
        ...
        for a in model.schedule.agents:
            if (getattr(a, "isolation") ==True):
                if a not in model.iso:
                    total +=1
...
def plot_SEIR(df_out, title, output_path = None):
    today = datemethod.strftime(datetime.utcnow(), '%Y%m%dZ%H%M%S')
    plot_name = 'SEIR_' + today + '_.png'
    ax = plt.subplot(111)
    for column in list(df_out.columns):
        if (column != 'R0') and (column != 'severe_cases'):
            ax.plot(df_out[column], label=column)
    plt.title(title +' - SEIR')
    plt.xlabel('Day')
    plt.ylabel('Population')
    ax.legend()
    if output_path != None:
